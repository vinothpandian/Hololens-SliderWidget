...
namespace HoloToolkit.Unity
{
    ...
    {
        //List<string> ManualDeploymentAddresses = new List<string>() { "10.16.1.127" };
        List<string> ManualDeploymentAddresses = new List<string>() { "10.0.1.23" };
        ...
        private void OnGUI()
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                GUILayout.FlexibleSpace();
                GUI.enabled = ShouldBuildSLNBeEnabled;
                if (GUILayout.Button("Build SLN, Build APPX, then Install & Run", GUILayout.Width(buttonWidth_Half)))
                {
                    // Build SLN
            ...
            else
            {
                ...
                addressesToPresent = addressesToPresent.Concat(ManualDeploymentAddresses).ToList();
        ...
        void BuildAndRun(string appName)
        {
            ...
            LaunchAppOnIPs(BuildDeployPrefs.TargetIPs);
        }
